/**
 * **************************************************************************** Compilation: javac
 * SegmentTree.java Execution: java SegmentTree
 *
 * <p>A segment tree data structure.
 *
 * <p>****************************************************************************
 */
 ...
 *
 * <p>Also it has been develop with {@code LazyPropagation} for range updates, which means when you
 ...
 * <p>Example:
 *
 * <p>SegmentTreeHeap st = new SegmentTreeHeap(new Integer[]{1,3,4,2,1, -2, 4}); st.update(0,3, 1)
...
public class SegmentTree {
  ...
  public SegmentTree(int[] array) {
    ...
    size =
        (int)
            (2 * Math.pow(2.0, Math.floor((Math.log((double) array.length) / Math.log(2.0)) + 1)));
    heap = new Node[size];
  ...
  private void update(int v, int from, int to, int value) {
    ...
    /**
     * If the updating-range contains the portion of the current Node We lazily update it. This
     * means We do NOT update each position of the vector, but update only some temporal values into
     * the Node; such values into the Node will be propagated down to its children only when they
     * need to.
     */
    if (contains(from, to, n.from, n.to)) {
      change(n, value);
    ...
    if (intersects(from, to, n.from, n.to)) {
       ...
       * temporally/lazily saved into this Node to its children So that when We visit them the
   ...
   * Initializes the array with [a, b, c ...] rsq a b Range Sum Query for the range [a, b] rmq a b
   * Range Min Query for the range [a, b] up a b v Update the [a,b] portion of the array with value
   * v. exit
   *
   * <p>Example: init set 1 2 3 4 5 6 rsq 1 3 Sum from 1 to 3 = 6 rmq 1 3 Min from 1 to 3 = 1 input
   * up 1 3 [3,2,3,4,5,6]
   *
