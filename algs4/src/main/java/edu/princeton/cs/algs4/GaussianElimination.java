/**
 * **************************************************************************** Compilation: javac
 * GaussianElimination.java Execution: java GaussianElimination m n Dependencies: StdOut.java
 *
 * <p>Gaussian elimination with partial pivoting for m-by-n system.
 *
 * <p>% java GaussianElimination m n -1.000000 2.000000 2.000000
 *
...
/**
 * The {@code GaussianElimination} data type provides methods to solve a linear system of equations
 * <em>Ax</em> = <em>b</em>, where <em>A</em> is an <em>m</em>-by-<em>n</em> matrix and <em>b</em>
 * is a length <em>n</em> vector.
 *
 * <p>This is a bare-bones implementation that uses Gaussian elimination with partial pivoting. See
 * <a href =
 * "https://algs4.cs.princeton.edu/99scientific/GaussianEliminationLite.java.html">GaussianEliminationLite.java</a>
 * for a stripped-down version that assumes the matrix <em>A</em> is square and nonsingular. See
 * {@link GaussJordanElimination} for an alternate implementation that uses Gauss-Jordan
 * elimination. For an industrial-strength numerical linear algebra library, see <a href =
 * "http://math.nist.gov/javanumerics/jama/">JAMA</a>.
 *
...
public class GaussianElimination {
   ...
   * <em>m</em>-by-<em>n</em> matrix and <em>b</em> is a length <em>m</em> vector.
   *
   * @param A the <em>m</em>-by-<em>n</em> constraint matrix
   * @param b the length <em>m</em> right-hand-side vector
   * @throws IllegalArgumentException if the dimensions disagree, i.e., the length of {@code b} does
   *     not equal {@code m}
   */
  ...
  public double[] primal() {
    ...
    for (int i = Math.min(n - 1, m - 1); i >= 0; i--) {
      ...
      if (Math.abs(a[i][i]) > EPSILON) x[i] = (a[i][n] - sum) / a[i][i];
      else if (Math.abs(a[i][n] - sum) > EPSILON) return null;
    }
    ...
    for (int i = n; i < m; i++) {
      ...
      if (Math.abs(a[i][n] - sum) > EPSILON) return null;
    }
    return x;
  }
