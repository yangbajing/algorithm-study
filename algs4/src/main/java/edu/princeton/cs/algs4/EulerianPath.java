/**
 * **************************************************************************** Compilation: javac
 * EulerianPath.java Execution: java EulerianPath V E Dependencies: Graph.java Stack.java
 * StdOut.java
 *
 * <p>Find an Eulerian path in a graph, if one exists.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code EulerianPath} class represents a data type for finding an Eulerian path in a graph. An
 * <em>Eulerian path</em> is a path (not necessarily simple) that uses every edge in the graph
 * exactly once.
 *
 * <p>This implementation uses a nonrecursive depth-first search. The constructor takes
 * &Theta;(<em>E</em> + <em>V</em>) time in the worst case, where <em>E</em> is the number of edges
 * and <em>V</em> is the number of vertices. Each instance method takes &Theta;(1) time. It uses
 * &Theta;(<em>E</em> + <em>V</em>) extra space in the worst case (not including the digraph).
 *
 * <p>To compute Eulerian cycles in graphs, see {@link EulerianCycle}. To compute Eulerian cycles
 * and paths in digraphs, see {@link DirectedEulerianCycle} and {@link DirectedEulerianPath}.
 *
...
public class EulerianPath {
   ...
   *
   * @return {@code true} if the graph has an Eulerian path; {@code false} otherwise
   */
  ...
  private static boolean satisfiesNecessaryAndSufficientConditions(Graph G) {
    ...
    int oddDegreeVertices = 0;
    for (int v = 0; v < G.V(); v++) if (G.degree(v) % 2 != 0) oddDegreeVertices++;
