/**
 * **************************************************************************** Compilation: javac
 * DigraphGenerator.java Execution: java DigraphGenerator V E Dependencies: Digraph.java
 *
 * <p>A digraph generator.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code DigraphGenerator} class provides static methods for creating various digraphs,
...
public class DigraphGenerator {
  ...
  /**
   * Returns a random simple digraph on {@code V} vertices, with an edge between any two vertices
   * with probability {@code p}. This is sometimes referred to as the Erdos-Renyi random digraph
   * model. This implementations takes time propotional to V^2 (even if {@code p} is small).
   *
   * @param V the number of vertices
   * @param p the probability of choosing an edge
   * @return a random simple digraph on {@code V} vertices, with an edge between any two vertices
  ...
  /**
   * Returns the complete digraph on {@code V} vertices. In a complete digraph, every pair of
   * distinct vertices is connected by two antiparallel edges. There are {@code V*(V-1)} edges.
   *
   * @param V the number of vertices
  ...
  public static Digraph complete(int V) {
    Digraph G = new Digraph(V);
    for (int v = 0; v < V; v++) for (int w = 0; w < V; w++) if (v != w) G.addEdge(v, w);
    return G;
  }

  /**
   * Returns a random simple DAG containing {@code V} vertices and {@code E} edges. Note: it is not
   * uniformly selected at random among all such DAGs.
   *
   * @param V the number of vertices
   * @param E the number of vertices
  ...
  /**
   * Returns a random tournament digraph on {@code V} vertices. A tournament digraph is a digraph in
   * which, for every pair of vertices, there is one and only one directed edge connecting them. A
   * tournament is an oriented complete graph.
   *
   * @param V the number of vertices
  ...
  /**
   * Returns a complete rooted-in DAG on {@code V} vertices. A rooted in-tree is a DAG in which
   * there is a single vertex reachable from every other vertex. A complete rooted in-DAG has
   * V*(V-1)/2 edges.
   *
   * @param V the number of vertices
  ...
  public static Digraph completeRootedInDAG(int V) {
    ...
    for (int i = 0; i < V; i++) vertices[i] = i;
    StdRandom.shuffle(vertices);
    for (int i = 0; i < V; i++) for (int j = i + 1; j < V; j++) G.addEdge(vertices[i], vertices[j]);

    return G;
  }

  /**
   * Returns a random rooted-in DAG on {@code V} vertices and {@code E} edges. A rooted in-tree is a
   * DAG in which there is a single vertex reachable from every other vertex. The DAG returned is
   * not chosen uniformly at random among all such DAGs.
   *
   * @param V the number of vertices
  ...
  /**
   * Returns a complete rooted-out DAG on {@code V} vertices. A rooted out-tree is a DAG in which
   * every vertex is reachable from a single vertex. A complete rooted in-DAG has V*(V-1)/2 edges.
   *
   * @param V the number of vertices
  ...
  public static Digraph completeRootedOutDAG(int V) {
    ...
    for (int i = 0; i < V; i++) vertices[i] = i;
    StdRandom.shuffle(vertices);
    for (int i = 0; i < V; i++) for (int j = i + 1; j < V; j++) G.addEdge(vertices[j], vertices[i]);

    return G;
  }

  /**
   * Returns a random rooted-out DAG on {@code V} vertices and {@code E} edges. A rooted out-tree is
   * a DAG in which every vertex is reachable from a single vertex. The DAG returned is not chosen
   * uniformly at random among all such DAGs.
   *
   * @param V the number of vertices
   * @param E the number of edges
  ...
  /**
   * Returns a random rooted-in tree on {@code V} vertices. A rooted in-tree is an oriented tree in
   * which there is a single vertex reachable from every other vertex. The tree returned is not
   * chosen uniformly at random among all such trees.
   *
   * @param V the number of vertices
  ...
  /**
   * Returns a random rooted-out tree on {@code V} vertices. A rooted out-tree is an oriented tree
   * in which each vertex is reachable from a single vertex. It is also known as a
   * <em>arborescence</em> or <em>branching</em>. The tree returned is not chosen uniformly at
   * random among all such trees.
   *
   * @param V the number of vertices
  ...
  /**
   * Returns a random simple digraph on {@code V} vertices, {@code E} edges and (at least) {@code c}
   * strong components. The vertices are randomly assigned integer labels between {@code 0} and
   * {@code c-1} (corresponding to strong components). Then, a strong component is creates among the
   * vertices with the same label. Next, random edges (either between two vertices with the same
   * labels or from a vetex with a smaller label to a vertex with a larger label). The number of
   * components will be equal to the number of distinct labels that are assigned to vertices.
   *
   * @param V the number of vertices
   * @param E the number of edges
   * @param c the (maximum) number of strong components
   * @return a random simple digraph on {@code V} vertices and {@code E} edges, with (at most)
   *     {@code c} strong components
  ...
  public static Digraph strong(int V, int E, int c) {
    ...
    int[] label = new int[V];
    for (int v = 0; v < V; v++) label[v] = StdRandom.uniform(c);
