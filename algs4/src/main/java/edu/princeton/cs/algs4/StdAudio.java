 ...
 *
 * <p>Limitations ----------- - Assumes the audio is monaural, little endian, with sampling rate of
 * 44,100 - check when reading .wav files from a .jar file ?
 *
 * <p>****************************************************************************
 */
 ...
 * audio.
 *
 * <p>The audio format uses a sampling rate of 44,100 Hz, 16-bit, monaural.
 *
...
public final class StdAudio {
  ...
  private static void init() {
    try {
      ...
      AudioFormat format =
          new AudioFormat((float) SAMPLE_RATE, BITS_PER_SAMPLE, MONO, SIGNED, LITTLE_ENDIAN);
   ...
   * values between -1.0 and +1.0. The audio file must be 16-bit with a sampling rate of 44,100. It
  ...
  public static double[] read(String filename) {
    ...
    if (audioFormat.getChannels() == MONO) {
      ...
      for (int i = 0; i < n / 2; i++) {
        // little endian, mono
        data[i] =
            ((short) (((bytes[2 * i + 1] & 0xFF) << 8) | (bytes[2 * i] & 0xFF)))
                / ((double) MAX_16_BIT);
      }
      return data;
    }
   ...
   * @throws IllegalArgumentException if {@code filename} extension is not {@code .wav} or {@code
   *     .au}
   */
