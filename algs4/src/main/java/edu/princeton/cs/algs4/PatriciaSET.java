/**
 * **************************************************************************** Compilation: javac
 * PatriciaSET.java Execution: java PatriciaSET Dependencies: StdOut.java StdRandom.java Queue.java
 * Data files: n/a
 *
 * <p>A set implementation based on PATRICIA.
 *
 * <p>% java PatriciaSET 1000000 1 Creating dataset (1000000 items)... Shuffling... Adding (1000000
...
/**
 * The {@code PatriciaSET} class provides an implementation of an unordered set, with the
 * restriction that the items (keys) are of class {@link java.lang.String}. It supports the usual
 * <em>add</em>, <em>contains</em>, <em>delete</em>, <em>size</em>, and <em>is-empty</em> methods.
 * It also provides an <em>iterator</em> method for iterating over all the elements in the set.
 *
 * <p>This unordered set class implements PATRICIA (Practical Algorithm to Retrieve Information
 * Coded In Alphanumeric). In spite of the acronym, string keys are not limited to alphanumeric
 * content. A key may possess any string value, with one exception: a zero-length string is not
 * permitted.
 *
 * <p>Unlike other generic set implementations that can accept a parameterized key type, this set
 * class can only accommodate keys of class {@link java.lang.String}. This unfortunate restriction
 * stems from a limitation in Java. Although Java provides excellent support for generic
 * programming, the current infrastructure somewhat limits generic collection implementations to
 * those that employ comparison-based or hash-based methods. PATRICIA does not employ comparisons or
 * hashing; instead, it relies on bit-test operations. Because Java does not furnish any generic
 * abstractions (or implementations) for bit-testing the contents of an object, providing support
 * for generic keys using PATRICIA does not seem practical.
 *
 ...
 *
 * <p>The analysis of PATRICIA is complicated. The theoretical wost-case performance for an
 * <em>add</em>, <em>contains</em>, or <em>delete</em> operation is <strong>O(N)</strong>, when
 ...
 * <em>add</em>, <em>contains</em>, or <em>delete</em> operation. Although this appears to put
 * PATRICIA on the same footing as binary trees, this time complexity represents the number of
 * single-bit test operations (under PATRICIA), and not full-key comparisons (as required by binary
 * trees). After the single-bit tests conclude, PATRICIA requires just one full-key comparison to
 * confirm the existence (or absence) of the key (per <em>add</em>, <em>contains</em>, or
...
public class PatriciaSET implements Iterable<String> {
  ...
  /**
   * Unit tests the {@code PatriciaSET} data type. This test fixture runs a series of tests on a
  ...
  public static void main(String[] args) {
    ...
    do {
      ...
      for (int i = 0; i < limitItem; i++) a[i] = Integer.toString(i, 16);

      StdOut.printf("Shuffling...\n");
      StdRandom.shuffle(a);

      StdOut.printf("Adding (%d items)...\n", limitItem);
      for (int i = 0; i < limitItem; i++) set.add(a[i]);
      ...
      for (int i = 0; i < limitDelete; i++) set.delete(a[i]);
      ...
      for (int i = countDelete; i < limitItem; i++) set.delete(a[i]);
      if (!set.isEmpty()) ok = false;
