/**
 * **************************************************************************** Compilation: javac
 * FlowEdge.java Execution: java FlowEdge Dependencies: StdOut.java
 *
 * <p>Capacitated edge with a flow in a flow network.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code FlowEdge} class represents a capacitated edge with a flow in a {@link FlowNetwork}.
 * Each edge consists of two integers (naming the two vertices), a real-valued capacity, and a
 * real-valued flow. The data type provides methods for accessing the two endpoints of the directed
 * edge and the weight. It also provides methods for changing the amount of flow on the edge and
...
public class FlowEdge {
  ...
  /**
   * Initializes an edge from vertex {@code v} to vertex {@code w} with the given {@code capacity}
   * and zero flow.
   *
   ...
   * @throws IllegalArgumentException unless {@code flow} is between {@code 0.0} and {@code
   *     capacity}.
   */
   ...
   *
   * @param vertex one endpoint of the edge
   * @return the residual capacity of the edge in the direction to the given vertex If {@code
   *     vertex} is the tail vertex, the residual capacity equals {@code capacity() - flow()}; if
   *     {@code vertex} is the head vertex, the residual capacity equals {@code flow()}.
   * @throws IllegalArgumentException if {@code vertex} is not one of the endpoints of the edge
   */
  ...
  /**
   * Increases the flow on the edge in the direction to the given vertex. If {@code vertex} is the
   * tail vertex, this increases the flow on the edge by {@code delta}; if {@code vertex} is the
   * head vertex, this decreases the flow on the edge by {@code delta}.
   *
   * @param vertex one endpoint of the edge
   * @param delta amount by which to increase flow
   * @throws IllegalArgumentException if {@code vertex} is not one of the endpoints of the edge
   * @throws IllegalArgumentException if {@code delta} makes the flow on on the edge either negative
   *     or larger than its capacity
