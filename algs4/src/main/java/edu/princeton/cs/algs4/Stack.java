/**
 * **************************************************************************** Compilation: javac
 * Stack.java Execution: java Stack < input.txt Dependencies: StdIn.java StdOut.java Data files:
 * https://algs4.cs.princeton.edu/13stacks/tobe.txt
 *
 * <p>A generic stack, implemented using a singly linked list. Each stack element is of type Item.
 *
 * <p>This version uses a static nested class Node (to save 8 bytes per Node), whereas the version
 * in the textbook uses a non-static nested class (for simplicity).
 *
 * <p>% more tobe.txt to be or not to - be - - that - - - is
 *
 * <p>% java Stack < tobe.txt to be not that or be (2 left on stack)
 *
 * <p>****************************************************************************
 */
 ...
 * nodes. See {@link LinkedStack} for the version from the textbook that uses a non-static nested
 * class. See {@link ResizingArrayStack} for a version that uses a resizing array. The
 * <em>push</em>, <em>pop</em>, <em>peek</em>, <em>size</em>, and <em>is-empty</em> operations all
