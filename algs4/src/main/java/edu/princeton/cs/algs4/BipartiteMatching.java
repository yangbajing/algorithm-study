/**
 * **************************************************************************** Compilation: javac
 * BipartiteMatching.java Execution: java BipartiteMatching V1 V2 E Dependencies: BipartiteX.java
 *
 * <p>Find a maximum cardinality matching (and minimum cardinality vertex cover) in a bipartite
 * graph using the alternating path algorithm.
 *
 * <p>****************************************************************************
 */
 ...
 *
 * <p>This implementation uses the <em>alternating-path algorithm</em>. It is equivalent to reducing
 * to the maximum-flow problem and running the augmenting-path algorithm on the resulting flow
 * network, but it does so with less overhead. The order of growth of the running time in the worst
 * case is (<em>E</em> + <em>V</em>) <em>V</em>, where <em>E</em> is the number of edges and
 * <em>V</em> is the number of vertices in the graph. It uses extra space (not including the graph)
 * proportional to <em>V</em>.
 *
 * <p>See also {@link HopcroftKarp}, which solves the problem in O(<em>E</em> sqrt(<em>V</em>))
 * using the Hopcroft-Karp algorithm and <a href =
