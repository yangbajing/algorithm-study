/**
 * **************************************************************************** Compilation: javac
 * MSD.java Execution: java MSD < input.txt Dependencies: StdIn.java StdOut.java Data files:
 * https://algs4.cs.princeton.edu/51radix/words3.txt
 * https://algs4.cs.princeton.edu/51radix/shells.txt
 *
 * <p>Sort an array of strings or integers using MSD radix sort.
 *
 * <p>% java MSD < shells.txt are by sea seashells seashells sells sells she she shells shore surely
 * the the
 *
 * <p>****************************************************************************
 */
...
public class MSD {
  ...
  private static void sort(int[] a, int lo, int hi, int d, int[] aux) {
    ...
    /**
     * *********** BUGGGY CODE. // for most significant byte, 0x80-0xFF comes before 0x00-0x7F if (d
     * == 0) { int shift1 = count[R] - count[R/2]; int shift2 = count[R/2]; for (int r = 0; r < R/2;
     * r++) count[r] += shift1; for (int r = R/2; r < R; r++) count[r] -= shift2; }
     * **********************************
     */
    // distribute
    for (int i = lo; i <= hi; i++) {
  ...
  private static void insertion(int[] a, int lo, int hi, int d) {
    for (int i = lo; i <= hi; i++)
      for (int j = i; j > lo && a[j] < a[j - 1]; j--) exch(a, j, j - 1);
  }
  ...
  /**
   * Reads in a sequence of extended ASCII strings from standard input; MSD radix sorts them; and
