/**
 * **************************************************************************** Compilation: javac
 * ST.java Execution: java ST < input.txt Dependencies: StdIn.java StdOut.java Data files:
 * https://algs4.cs.princeton.edu/35applications/tinyST.txt
 *
 * <p>Sorted symbol table implementation using a java.util.TreeMap. Does not allow duplicates.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code ST} class represents an ordered symbol table of generic key-value pairs. It supports
 ...
 * <em>maximum</em>, <em>floor</em>, and <em>ceiling</em>. It also provides a <em>keys</em> method
 * for iterating over all of the keys. A symbol table implements the <em>associative array</em>
 * abstraction: when associating a value with a key that is already in the symbol table, the
 ...
 * key to {@code null} is equivalent to deleting the key from the symbol table.
 *
 * <p>This implementation uses a red-black BST. It requires that the key type implements the {@code
 * Comparable} interface and calls the {@code compareTo()} and method to compare two keys. It does
 * not call either {@code equals()} or {@code hashCode()}. The <em>put</em>, <em>get</em>,
 * <em>contains</em>, <em>remove</em>, <em>minimum</em>, <em>maximum</em>, <em>ceiling</em>, and
 * <em>floor</em> operations each take &Theta;(log <em>n</em>) time in the worst case, where
 * <em>n</em> is the number of key-value pairs in the symbol table. The <em>size</em> and
 * <em>is-empty</em> operations take &Theta;(1) time. Construction takes &Theta;(1) time.
 *
...
public class ST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
   ...
   *
   * @param key the key
   * @return the value associated with the given key if the key is in this symbol table; {@code
   ...
   * this symbol table). This is equivalent to {@code remove()}, but we plan to deprecate {@code
   * delete()}.
   *
   * @param key the key
   * @throws IllegalArgumentException if {@code key} is {@code null}
  ...
  /**
   * Removes the specified key and its associated value from this symbol table (if the key is in
   * this symbol table). This is equivalent to {@code delete()}, but we plan to deprecate {@code
   * delete()}.
   *
   * @param key the key
   * @throws IllegalArgumentException if {@code key} is {@code null}
   ...
   *
   * <p>To iterate over all of the keys in the symbol table named {@code st}, use the foreach
  ...
  /**
   * Returns all of the keys in this symbol table. To iterate over all of the keys in a symbol table
   * named {@code st}, use the foreach notation: {@code for (Key key : st)}.
   *
   * <p>This method is provided for backward compatibility with the version from <em>Introduction to
   * Programming in Java: An Interdisciplinary Approach.</em>
   *
