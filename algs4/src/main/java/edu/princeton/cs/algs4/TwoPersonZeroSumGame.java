/**
 * **************************************************************************** Compilation: javac
 * TwoPersonZeroSumGame.java Execution: java TwoPersonZeroSumGame m n Dependencies:
 * LinearProgramming.java StdOut.java
 *
 * <p>Solve an m-by-n two-person zero-sum game by reducing it to linear programming. Assuming A is a
 * strictly positive payoff matrix, the optimal row and column player strategies are x* an y*,
 * scaled to be probability distributions.
 *
 * <p>(P) max y^T 1 (D) min 1^T x s.t A^T y <= 1 s.t A x >= 1 y >= 0 x >= 0
 *
 * <p>Row player is x, column player is y.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code TwoPersonZeroSumGame} class represents a data type for computing optimal row and
 * column strategies to two-person zero-sum games.
 *
 * <p>This implementation solves an <em>m</em>-by-<em>n</em> two-person zero-sum game by reducing it
 * to a linear programming problem. Assuming the payoff matrix <em>A</em> is strictly positive, the
 * optimal row and column player strategies x* and y* are obtained by solving the following primal
 * and dual pair of linear programs, scaling the results to be probability distributions.
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * <p>If the payoff matrix <em>A</em> has any negative entries, we add the same constant to every
 * entry so that every entry is positive. This increases the value of the game by that constant, but
 * does not change solutions to the two-person zero-sum game.
 *
 * <p>This implementation is not suitable for large inputs, as it calls a bare-bones linear
 * programming solver that is neither fast nor robust with respect to floating-point roundoff error.
 *
...
public class TwoPersonZeroSumGame {
  ...
  public TwoPersonZeroSumGame(double[][] payoff) {
    ...
    for (int j = 0; j < n; j++) c[j] = 1.0;

    // find smallest entry
    constant = Double.POSITIVE_INFINITY;
    for (int i = 0; i < m; i++)
      for (int j = 0; j < n; j++) if (payoff[i][j] < constant) constant = payoff[i][j];

    // add constant  to every entry to make strictly positive
    if (constant <= 0) constant = -constant + 1;
    else constant = 0;
    for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) A[i][j] = payoff[i][j] + constant;
  ...
  private static void test(String description, double[][] payoff) {
    ...
    StdOut.print("x[] = [");
    for (int j = 0; j < n - 1; j++) StdOut.printf("%8.4f, ", x[j]);
    StdOut.printf("%8.4f]\n", x[n - 1]);

    StdOut.print("y[] = [");
    for (int i = 0; i < m - 1; i++) StdOut.printf("%8.4f, ", y[i]);
    StdOut.printf("%8.4f]\n", y[m - 1]);
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < m; i++)
      for (int j = 0; j < n; j++) payoff[i][j] = StdRandom.uniform(-0.5, 0.5);
