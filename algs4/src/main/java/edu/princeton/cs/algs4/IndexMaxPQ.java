/**
 * **************************************************************************** Compilation: javac
 * IndexMaxPQ.java Execution: java IndexMaxPQ Dependencies: StdOut.java
 *
 * <p>Maximum-oriented indexed PQ implementation using a binary heap.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code IndexMaxPQ} class represents an indexed priority queue of generic keys. It supports
 * the usual <em>insert</em> and <em>delete-the-maximum</em> operations, along with <em>delete</em>
 * and <em>change-the-key</em> methods. In order to let the client refer to items on the priority
 * queue, an integer between {@code 0} and {@code maxN - 1} is associated with each key—the client
 * uses this integer to specify which key to delete or change. It also supports methods for peeking
 * at a maximum key, testing if the priority queue is empty, and iterating through the keys.
 *
 * <p>This implementation uses a <em>binary heap</em> along with an array to associate keys with
 * integers in the given range. The <em>insert</em>, <em>delete-the-maximum</em>, <em>delete</em>,
 * <em>change-key</em>, <em>decrease-key</em>, and <em>increase-key</em> operations take &Theta;(log
 * <em>n</em>) time in the worst case, where <em>n</em> is the number of elements in the priority
 * queue. Construction takes time proportional to the specified capacity.
 *
 * <p>For additional documentation, see <a href="https://algs4.cs.princeton.edu/24pq">Section
 * 2.4</a> of <i>Algorithms, 4th Edition</i> by Robert Sedgewick and Kevin Wayne.
 *
...
public class IndexMaxPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
  ...
  /**
   * Returns an iterator that iterates over the keys on the priority queue in descending order. The
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < strings.length; i++) {
      if (StdRandom.uniform() < 0.5) pq.increaseKey(i, strings[i] + strings[i]);
