/**
 * **************************************************************************** Compilation: javac
 * BipartiteX.java Execution: java Bipartite V E F Dependencies: Graph.java
 *
 * <p>Given a graph, find either (i) a bipartition or (ii) an odd-length cycle. Runs in O(E + V)
 * time.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code BipartiteX} class represents a data type for determining whether an undirected graph
 * is <em>bipartite</em> or whether it has an <em>odd-length cycle</em>. A graph is bipartite if and
 * only if it has no odd-length cycle. The <em>isBipartite</em> operation determines whether the
 * graph is bipartite. If so, the <em>color</em> operation determines a bipartition; if not, the
 * <em>oddCycle</em> operation determines a cycle with an odd number of edges.
 *
 * <p>This implementation uses <em>breadth-first search</em> and is nonrecursive. The constructor
 * takes &Theta;(<em>V</em> + <em>E</em>) time in in the worst case, where <em>V</em> is the number
 * of vertices and <em>E</em> is the number of edges. Each instance method takes &Theta;(1) time. It
 * uses &Theta;(<em>V</em>) extra space (not including the graph). See {@link Bipartite} for a
 * recursive version that uses depth-first search.
 *
...
public class BipartiteX {
   ...
   *
   * @param v the vertex
   * @return the side of the bipartition that vertex {@code v} is on; two vertices are in the same
   *     side of the bipartition if and only if they have the same color
   * @throws IllegalArgumentException unless {@code 0 <= v < V}
   * @throws UnsupportedOperationException if this method is called when the graph is not bipartite
   */
   ...
   *
   * @return an odd-length cycle if the graph is not bipartite (and hence has an odd-length cycle),
