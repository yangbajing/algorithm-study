/**
 * **************************************************************************** Compilation: javac
 * Inversions.java Execution: java Inversions < input.txt Dependencies: StdIn.java StdOut.java
 *
 * <p>Read array of n integers and count number of inversions in n log n time.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code Inversions} class provides static methods to count the number of <em>inversions</em>
 * in either an array of integers or comparables. An inversion in an array {@code a[]} is a pair of
 * indicies {@code i} and {@code j} such that {@code i < j} and {@code a[i] > a[j]}.
 *
 * <p>This implementation uses a generalization of mergesort. The <em>count</em> operation takes
 * &Theta;(<em>n</em> log <em>n</em>) time to count the number of inversions in any array of length
...
public class Inversions {
   ...
   *
   * @param a the array
   * @return the number of inversions in the array. An inversion is a pair of indicies {@code i} and
  ...
  private static <Key extends Comparable<Key>> long merge(
      Key[] a, Key[] aux, int lo, int mid, int hi) {
    long inversions = 0;
  ...
  // return the number of inversions in the subarray b[lo..hi]
  // side effect b[lo..hi] is rearranged in ascending order
  private static <Key extends Comparable<Key>> long count(
      Key[] a, Key[] b, Key[] aux, int lo, int hi) {
    long inversions = 0;
    if (hi <= lo) return 0;
    int mid = lo + (hi - lo) / 2;
    inversions += count(a, b, aux, lo, mid);
   ...
   * @return the number of inversions in the array. An inversion is a pair of indicies {@code i} and
   *     {@code j} such that {@code i < j} and {@code a[i].compareTo(a[j]) > 0}.
   */
  ...
  private static <Key extends Comparable<Key>> long brute(Key[] a, int lo, int hi) {
    long inversions = 0;
    for (int i = lo; i <= hi; i++)
      for (int j = i + 1; j <= hi; j++) if (less(a[j], a[i])) inversions++;
    return inversions;
  }
  ...
  private static long brute(int[] a, int lo, int hi) {
    long inversions = 0;
    for (int i = lo; i <= hi; i++) for (int j = i + 1; j <= hi; j++) if (a[j] < a[i]) inversions++;
    return inversions;
  }

  /**
   * Reads a sequence of integers from standard input and prints the number of inversions to
   * standard output.
   *
