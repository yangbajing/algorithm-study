/**
 * **************************************************************************** Compilation: javac
 * FarthestPair.java Execution: java FarthestPair < input.txt Dependencies: GrahamScan.java
 * Point2D.java Data files: https://algs4.cs.princeton.edu/99hull/rs1423.txt
 * https://algs4.cs.princeton.edu/99hull/kw1260.txt
 *
 * <p>Given a set of n points in the plane, find the farthest pair (equivalently, compute the
 * diameter of the set of points).
 *
 * <p>Computes the convex hull of the set of points and using the rotating calipers method to find
 * all antipodal point pairs and the farthest pair.
 *
 * <p>% java FarthestPair < input100.txt 42697.98170874122 from (32011.0, 3140.0) to (822.0,
 * 32301.0)
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code FarthestPair} data type computes the farthest pair of points in a set of <em>n</em>
 * points in the plane and provides accessor methods for getting the farthest pair of points and the
 * distance between them. The distance between two points is their Euclidean distance.
 *
 * <p>This implementation computes the convex hull of the set of points and uses the rotating
 * calipers method to find all antipodal point pairs and the farthest pair. It runs in O(<em>n</em>
 * log <em>n</em>) time in the worst case and uses O(<em>N</em>) extra space. See also {@link
 * ClosestPair} and {@link GrahamScan}.
 *
...
public class FarthestPair {
  ...
  public FarthestPair(Point2D[] points) {
    ...
    int k = 2;
    while (Point2D.area2(hull[m], hull[1], hull[k + 1])
   ...
   *
   * @return one of the two points in the farthest pair of points; {@code null} if no such point
   ...
   *
   * @return the other point in the farthest pair of points {@code null} if no such point (because
  ...
  /**
   * Returns the Eucliden distance between the farthest pair of points. This quantity is also known
   * as the <em>diameter</em> of the set of points.
   *
   * @return the Euclidean distance between the farthest pair of points {@code
  ...
  /**
   * Unit tests the {@code FarthestPair} data type. Reads in an integer {@code n} and {@code n}
   * points (specified by their <em>x</em>- and <em>y</em>-coordinates) from standard input;
   * computes a farthest pair of points; and prints the pair to standard output.
   *
