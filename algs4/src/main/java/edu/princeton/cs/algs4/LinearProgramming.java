/**
 * **************************************************************************** Compilation: javac
 * LinearProgramming.java Execution: java LinearProgramming m n Dependencies: StdOut.java
 *
 * <p>Given an m-by-n matrix A, an m-length vector b, and an n-length vector c, solve the LP { max
 * cx : Ax <= b, x >= 0 }. Assumes that b >= 0 so that x = 0 is a basic feasible solution.
 *
 * <p>Creates an (m+1)-by-(n+m+1) simplex tableaux with the RHS in column m+n, the objective
 * function in row m, and slack variables in columns m through m+n-1.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code LinearProgramming} class represents a data type for solving a linear program of the
 * form { max cx : Ax &le; b, x &ge; 0 }, where A is a m-by-n matrix, b is an m-length vector, and c
 * is an n-length vector. For simplicity, we assume that A is of full rank and that b &ge; 0 so that
 * x = 0 is a basic feasible soution.
 *
 * <p>The data type supplies methods for determining the optimal primal and dual solutions.
 *
 * <p>This is a bare-bones implementation of the <em>simplex algorithm</em>. It uses Bland's rule to
 * determing the entering and leaving variables. It is not suitable for use on large inputs. It is
 * also not robust in the presence of floating-point roundoff error.
 *
...
public class LinearProgramming {
  ...
  /**
   * Determines an optimal solution to the linear program { max cx : Ax &le; b, x &ge; 0 }, where A
   * is a m-by-n matrix, b is an m-length vector, and c is an n-length vector.
   *
  ...
  private int bland() {
    for (int j = 0; j < m + n; j++) if (a[m][j] > 0) return j;
    return -1; // optimal
  }
  ...
  private int dantzig() {
    int q = 0;
    for (int j = 1; j < m + n; j++) if (a[m][j] > a[m][q]) q = j;
  ...
  public double[] primal() {
    ...
    for (int i = 0; i < m; i++) if (basis[i] < n) x[basis[i]] = a[i][m + n];
    return x;
  }
  ...
  private boolean isOptimal(double[] b, double[] c) {
    ...
    double value1 = 0.0;
    for (int j = 0; j < x.length; j++) value1 += c[j] * x[j];
    double value2 = 0.0;
    for (int i = 0; i < y.length; i++) value2 += y[i] * b[i];
  ...
  private static void test(double[][] A, double[] b, double[] c) {
    ...
    double[] x = lp.primal();
    for (int i = 0; i < x.length; i++) StdOut.println("x[" + i + "] = " + x[i]);
    double[] y = lp.dual();
    for (int j = 0; j < y.length; j++) StdOut.println("y[" + j + "] = " + y[j]);
  }
  ...
  public static void main(String[] args) {
    ...
    for (int j = 0; j < n; j++) c[j] = StdRandom.uniform(1000);
    for (int i = 0; i < m; i++) b[i] = StdRandom.uniform(1000);
    for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) A[i][j] = StdRandom.uniform(100);
