/**
 * **************************************************************************** Compilation: javac
 * Vector.java Execution: java Vector Dependencies: StdOut.java
 *
 * <p>Implementation of a vector of real numbers.
 *
 * <p>This class is implemented to be immutable: once the client program initialize a Vector, it
 * cannot change any of its fields (d or data[i]) either directly or indirectly. Immutability is a
 * very desirable feature of a data type.
 *
 * <p>% java Vector x = [ 1.0 2.0 3.0 4.0 ] y = [ 5.0 2.0 4.0 1.0 ] z = [ 6.0 4.0 7.0 5.0 ] 10z = [
 * 60.0 40.0 70.0 50.0 ] |x| = 5.477225575051661 <x, y> = 25.0
 *
 * <p>Note that Vector is also the name of an unrelated Java library class in the package java.util.
 *
 * <p>****************************************************************************
 */
...
/**
 * The {@code Vector} class represents a <em>d</em>-dimensional Euclidean vector. Vectors are
 * immutable: their values cannot be changed after they are created. It includes methods for
 * addition, subtraction, dot product, scalar product, unit vector, Euclidean norm, and the
...
public class Vector {
  ...
  /**
   * Initializes a vector from either an array or a vararg list. The vararg syntax supports a
   * constructor that takes a variable number of arugments such as Vector x = new Vector(1.0, 2.0,
   * 3.0, 4.0).
   *
  ...
  public double dot(Vector that) {
    ...
    double sum = 0.0;
    for (int i = 0; i < d; i++) sum = sum + (this.data[i] * that.data[i]);
    return sum;
  }
  ...
  public Vector plus(Vector that) {
    ...
    for (int i = 0; i < d; i++) c.data[i] = this.data[i] + that.data[i];
    return c;
  }
   ...
   *     separates by single spaces
   */
  public String toString() {
    StringBuilder s = new StringBuilder();
    for (int i = 0; i < d; i++) s.append(data[i] + " ");
    return s.toString();
  }
