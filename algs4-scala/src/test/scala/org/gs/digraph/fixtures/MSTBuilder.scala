...
trait EdgeWeightedDigraphBuilder extends DirectedEdgeBuilder {
  def buildGraph(uri: String): EdgeWeightedDigraph = {
    val managedResource = readURI(uri)
    val tuple = managedResource.loan(readFileToTuple)
