...
class FlowEdge(v: Int, w: Int, val capacity: Double) extends BaseDirectedEdge {
  require(v >= 0 && w >= 0 && capacity >= 0 && !capacity.isNaN, "s invalid arg(s) v:$v w:$w capacity$capacity")
  ...
  def this(v: Int, w: Int, capacity: Double, flow: Double) {
    this(v, w, capacity)
    require(_flow >= 0 && flow <= capacity, s"flow:${_flow} must not be negative and can't exceed capacity:$capacity")
    _flow = flow
  }
  ...
  def addResidualFlowTo(vertex: Int, delta: Double): Unit = {
    ...
    assert(_flow >= 0 && flow <= capacity, s"flow:${_flow} must not be negative and can't exceed capacity:$capacity")
  }
}
