...
class RedBlackBST[A, B](implicit ord: Ordering[A]) {
  ...
  def deleteMax(): Unit = {
    def deleteMax(h: Node[A, B]): Node[A, B] = {
      ...
      if (j.right == null) null.asInstanceOf[Node[A, B]]
      else {
  ...
  def ceiling(key: A): A = {
    def loop(x: Node[A, B]): Node[A, B] =
      if (x == null) null.asInstanceOf[Node[A, B]]
      else {
  ...
  def isBST(): Boolean = {
    def loop(x: Node[A, B], min: A, max: A): Boolean = {
      if (x == null) true
        ...
        else if (max != null && ord.compare(x.key, max) >= 0) false
        else {
